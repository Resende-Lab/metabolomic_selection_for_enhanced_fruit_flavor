...
set.seed(1234)
if (species == "tomato"){
  tom = read.csv("data/tom_imputed.csv",check.names = F)
} else {
  tom = read.csv("data/bb_imputed.csv",check.names = F)
}
...
sft$powerEstimate
# Plot the results:
pdf(sprintf("%s_%s_Power.pdf",species,type), width = 9, height = 5);
  sizeGrWindow(9, 5)
  par(mfrow = c(1,2));
  cex1 = 0.9;
...
net = blockwiseModules(tom, power = sft$powerEstimate, networkType=type,
                       TOMType = type, minModuleSize = 3, maxBlockSize = 181,
                       reassignThreshold = 0, mergeCutHeight = 0.2,
                       numericLabels = TRUE, pamRespectsDendro = FALSE,
                       saveTOMs = TRUE,
                       saveTOMFileBase = sprintf("%s_TOM",species),verbose = 5)
...
cyt = exportNetworkToCytoscape(modTOM,
                               edgeFile = paste(species,"_",type,"CytoscapeInput-edges-", length(modules), "modules.txt", sep=""),
                               nodeFile = paste(species,"_",type,"CytoscapeInput-nodes-", length(modules), "modules.txt", sep=""),
                               weighted = TRUE,
                               threshold = 0.02,
                               nodeNames = modProbes,
